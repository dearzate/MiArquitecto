...
namespace MITEncrypt
{
    public class clsAESWebPay
    {
        public string encrypInAES128(string strKey, string strText)
        {
            try
            {
                ...
                using (Aes myAes = Aes.Create())
                {
                    ...
                    return ConvertStringToHex(System.Convert.ToBase64String(resultado));
                }
            }
            catch (Exception e)
            {
        ...
        public string descrypInAES128(string strKey, string hexText)
        {
            try
            {

                string original = hexText;
                ...
                using (Aes myAes = Aes.Create())
                {
                    ...
                    var base64EncodedBytes = System.Convert.FromBase64String(ConvertHexToString(hexText));
                    byte[] IVAES128 = new byte[16];
                    Array.Copy(base64EncodedBytes, 0, IVAES128, 0, 16);
                    myAes.IV = IVAES128;

                    base64EncodedBytes = System.Convert.FromBase64String(ConvertHexToString(hexText));
                    xmlByte = new byte[base64EncodedBytes.Length - 16];
                    Array.Copy(base64EncodedBytes, 16, xmlByte, 0, base64EncodedBytes.Length - 16);
                    myAes.Key = key;

                    // Encrypt the string to an array of bytes.
                    return DecryptStringFromBytes_Aes128(xmlByte, myAes.Key, myAes.IV);
                }
